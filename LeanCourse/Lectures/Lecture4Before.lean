...
example {s : Set α} {t : Set β} {f : α → β} : f '' s ⊆ t ↔ s ⊆ f ⁻¹' t := by {
  ...
  · intro h y hy
    obtain ⟨x, x_in_sx, fx_eq_y ⟩:= hy
    subst y
    specialize h x_in_sx
    exact h
  }
...
def conditionalInverse (y : β)
  (h : ∃ x : α, f x = y) : α := Classical.choose h
...
lemma inv_of_bijective (hf : Bijective f) :
  ...
  use g
  constructor
  · refine rightInv_of_surjective f ?h.left.hf
    exact Bijective.surjective hf
  · refine leftInv_of_surjective f ?h.right.hf
